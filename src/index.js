...
ipcMain.handle(
  "db:search-by-keyword",
  async (event, { from, to, external_id, keywords = [] }) => {
    try {
      return await searchPosts(from, to, external_id, ...keywords);
    } catch (error) {
...
ipcMain.handle("web:scrape", async (event, { url, dateFromWeb, dateToWeb }) => {
  ...
  for (const post of posts) {
    ...
    console.log(`post added: ${`web_${domain}_${postIdum}`}`);
  }
