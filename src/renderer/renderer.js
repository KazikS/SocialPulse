...
async function loadStatistics(from, to, externalChannelId, platform) {
    ...
    while (attempts < 5) {
        channelId = await window.electron.invoke("db:get-channel-id", externalChannelId);
        if (channelId) break;
      ...
      if (!channelId) {
        console.error("Канал не найден после 5 попыток");
        return;
      }
    ...
    try {
        ...
        console.log(JSON.stringify(stats, null, 2), 'чодержание блока статистики');
...
async function searchPostsDB() {
    const url = document.getElementById("urlInputWeb").value;
    const dateFrom = document.getElementById("dateFromWeb").value;
    const dateTo = document.getElementById("dateToWeb").value;
    const keywords = document.getElementById("keywordInput").value.split(" ");
    const resBlockHtml = document.getElementById("keywordsResultWeb");

    console.log(new URL(url).hostname);
    const searchResults = await window.electron.invoke('db:search-by-keyword', {
        from: dateFrom,
        to: dateTo,
        external_id: new URL(url).hostname, // или channelId: 5
        keywords
    });

    // new URL($post.find(".post__name").attr("href"), baseUrl).href
    let html = "";
    for(const post of searchResults){
        html += `<p style="border-radius: 10px; border: 1px solid black; padding: 2%; margin-bottom: 1%;"><a href="${post.link}" style="text-decoration: none">${post.text}</a></p>`
    }
    resBlockHtml.innerHTML = `
    <h3>Новости, содержащие ключевые слова:</h3>
    ${html}
    `;
    ...
    console.log('Найдено записей:', searchResults.length);
}
...
window.onload = () => {
    ...
    document.getElementById("startAnalysis").addEventListener("click", startVkAnalysis);
    document.getElementById("cancelBtn").addEventListener("click", cancelVkAnalysis);
    document.getElementById("startAnalysisTG").addEventListener("click", startTgAnslysis);
    document.getElementById("startScrape").addEventListener("click", startWebScrapping);
    document.getElementById("searchKeywordBtn").addEventListener("click", searchPostsDB)
};
